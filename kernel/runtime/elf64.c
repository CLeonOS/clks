...
clks_bool clks_elf64_load(const void *image, u64 size, struct clks_elf64_loaded_image *out_loaded) {
    ...
    if (image_base == CLKS_NULL) {
        ...
        clks_log_bytes(CLKS_LOG_WARN, "ELF", "load span", span);
        clks_log_hex(CLKS_LOG_WARN, "ELF", "MIN_VADDR", min_vaddr);
        clks_log_hex(CLKS_LOG_WARN, "ELF", "MAX_VADDR_END", max_vaddr_end);
        clks_log_bytes(CLKS_LOG_WARN, "ELF", "heap total", (u64)heap_stats.total_bytes);
        clks_log_bytes(CLKS_LOG_WARN, "ELF", "heap used", (u64)heap_stats.used_bytes);
        clks_log_bytes(CLKS_LOG_WARN, "ELF", "heap free", (u64)heap_stats.free_bytes);
        return CLKS_FALSE;
    }
