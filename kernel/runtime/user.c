...
static clks_bool clks_user_parse_record(const char *line, struct clks_user_record *out_record) {
    ...
    if (clks_user_name_valid(name) == CLKS_FALSE || clks_strlen(hash) != CLKS_USER_HASH_HEX_LEN || home[0] != '/') {
        return CLKS_FALSE;
    }
...
static clks_bool clks_user_verify_password(const char *name, const char *password,
                                           struct clks_user_record *out_record) {
    struct clks_user_record record;
    char hash[CLKS_USER_HASH_HEX_LEN + 1U];
...
clks_bool clks_user_at(u64 index, struct clks_user_public_info *out_info) {
    ...
    if (CLKS_CFG_USER_SYSTEM == 0 || out_info == CLKS_NULL || clks_user_read_db(db, sizeof(db), &len) == CLKS_FALSE) {
        return CLKS_FALSE;
    }
...
clks_bool clks_user_path_read_allowed(const char *path) {
    ...
    if (CLKS_CFG_USER_SYSTEM == 0 || CLKS_CFG_USER_READ_POLICY == 0 || clks_user_disk_login_required == CLKS_FALSE) {
        return CLKS_TRUE;
    }
...
clks_bool clks_user_path_write_allowed(const char *path) {
    ...
    if (CLKS_CFG_USER_SYSTEM == 0 || CLKS_CFG_USER_WRITE_POLICY == 0 || clks_user_disk_login_required == CLKS_FALSE) {
        return CLKS_TRUE;
    }
